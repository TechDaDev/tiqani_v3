"""Tests for settlement eligibility checks."""
from decimal import Decimal
from django.test import TestCase
from django.contrib.auth import get_user_model
from contract.models import Contract
from wallet.models import PaymentIntent
from wallet.services import create_contract_payment_breakdown
from wallet.settlement_services import check_settlement_eligibility

User = get_user_model()


class SettlementEligibilityTest(TestCase):
    def setUp(self):
        self.client_user = User.objects.create_user(username="client", password="pass", role="client")
        self.tech_user = User.objects.create_user(username="tech", password="pass", role="technician")
        self.other_client = User.objects.create_user(username="other", password="pass", role="client")
        self.contract = self._make_completed_funded_contract()

    def test_eligible(self):
        eligible, reason = check_settlement_eligibility(self.contract, self.client_user)
        self.assertTrue(eligible, reason)

    def test_technician_cannot_release(self):
        eligible, reason = check_settlement_eligibility(self.contract, self.tech_user)
        self.assertFalse(eligible)
        self.assertIn("Only the contract client", reason)

    def test_unrelated_client_cannot_release(self):
        eligible, reason = check_settlement_eligibility(self.contract, self.other_client)
        self.assertFalse(eligible)

    def test_active_contract_ineligible(self):
        self.contract.status = "active"
        self.contract.save(update_fields=["status"])
        eligible, reason = check_settlement_eligibility(self.contract, self.client_user)
        self.assertFalse(eligible)

    def test_unfunded_contract_ineligible(self):
        PaymentIntent.objects.filter(contract=self.contract).delete()
        eligible, reason = check_settlement_eligibility(self.contract, self.client_user)
        self.assertFalse(eligible)

    def _make_completed_funded_contract(self):
        from accounts.models import ClientProfile, TechnicianProfile
        cp = ClientProfile.objects.create(user=self.client_user)
        tp = TechnicianProfile.objects.create(user=self.tech_user)
        c = Contract.objects.create(
            client=cp, technician=tp,
            agreed_amount=Decimal("500000.00"),
            escrow_amount=Decimal("500000.00"),
            status="completed",
        )
        PaymentIntent.objects.create(
            contract=c, user=self.client_user,
            amount=Decimal("525000.00"),
            purpose=PaymentIntent.Purpose.CONTRACT_FUNDING,
            status=PaymentIntent.Status.PAID,
        )
        create_contract_payment_breakdown(c)
        return c
