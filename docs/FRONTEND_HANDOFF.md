...
## Phase 16 — Production Launch Additions

### OpenAPI Schema & Docs

| Endpoint | Description | Auth |
|---|---|---|
| `GET /api/schema/` | OpenAPI 3.0 schema (YAML/JSON) | Public in dev, admin in prod |
| `GET /api/docs/` | Swagger UI | Public in dev, admin in prod |
| `GET /api/redoc/` | ReDoc UI | Public in dev, admin in prod |

The schema is auto-generated by `drf-spectacular`. In production, set `API_DOCS_PUBLIC=False` to require admin authentication.

### Health Endpoints

| Endpoint | Response | Purpose |
|---|---|---|
| `GET /api/health/` | 200 + `{"status":"ok","service":"tiqani_v3","database":"ok"}` | Legacy alias for readiness |
| `GET /api/health/live/` | 200 + `{"status":"alive"}` | Liveness probe (no DB) |
| `GET /api/health/ready/` | 200 + `{"status":"ok","database":"ok"}` | Readiness probe (DB check) |
| `GET /api/health/deep/` | 200 + full status with Celery info | Deep health check |

### Request ID Header

Every API response includes `X-Request-ID` — an 8-character hex identifier.
You can pass your own ID via `X-Request-ID` request header for idempotency tracing.
...
|---|---|---|
| Anonymous | 10/minute | `THROTTLE_ANON` |
| Authenticated | 60/minute | `THROTTLE_USER` |
| Login | 5/minute | `THROTTLE_LOGIN_RATE` |
| OTP | 5/minute | `THROTTLE_OTP_RATE` |
| Password reset | 5/minute | `THROTTLE_PASSWORD_RESET_RATE` |
| Dealership finance | 30/minute | `THROTTLE_DEALERSHIP_FINANCE_RATE` |
| Wallet finance | 30/minute | `THROTTLE_WALLET_FINANCE_RATE` |
| Reviews | 60/minute | `THROTTLE_REVIEWS_RATE` |
| Notifications | 120/minute | `THROTTLE_NOTIFICATIONS_RATE` |
| Schema/docs | 20/minute | `THROTTLE_SCHEMA_RATE` |

### Upload / Media Behavior

- **Profile images**: Upload to profile endpoint. S3-backed in production.
- **Category icons**: Managed via admin.
- **Contract documents**: Uploaded during contract creation.
- **Dealership documents**: Guarantee documents, recharge proofs.
- **Private media**: Access via presigned S3 URLs.
- **Max profile image**: 2 MB
- **Max document**: 10 MB
- **Max proof file**: 5 MB

### Demo Accounts (if available)

- Admin: `admin` / contact team for password
- Client: demo client accounts available
- Technician: demo technician accounts available
- Dealership: demo financial agent accounts available

### Permission Matrix

See `docs/PERMISSION_MATRIX.md` for a full role-based access matrix.

### Known Limitations

- Rate limits are conservative — tune via env vars for production
- Schema/docs are public when `DEBUG=True` — set `API_DOCS_PUBLIC=False` in production
- WebSocket requires JWT token in query string
- Celery worker must be running for background jobs
- Redis required for WebSocket channel layer and Celery broker
- S3-compatible storage recommended for media in production
